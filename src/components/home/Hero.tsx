...
export function Hero() {
  return (
    <section className="relative w-full h-full min-h-[500px] flex flex-col overflow-hidden bg-[#0B1221]">
      {/* Mobile Stacked Layout (Text then Image) */}
      <div className="flex flex-col md:block h-full w-full">
        
        {/* Image Container - Moves below text on mobile, stays background on desktop */}
        <div className="order-2 md:absolute md:inset-0 h-[300px] md:h-full w-full bg-[#0B1221]">
          <img
            src="/hero-banner.png"
            alt="Premium authenticated sports memorabilia"
            className="h-full w-full object-contain md:object-right object-center"
          />
          {/* Desktop Gradient Overlay - Only visible on MD+ */}
          <div className="hidden md:block absolute inset-0 bg-gradient-to-r from-navy via-navy/40 to-transparent" />
        </div>

        {/* Content Container - Order 1 on mobile */}
        <div className="order-1 relative z-10 container mx-auto px-4 flex flex-col justify-center py-12 md:py-20 h-auto md:h-full">
          <div className="max-w-3xl space-y-5 sm:space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            {/* Offer-Led Badge */}
