...
export function ChatWidget() {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    // Get bot response
    setIsLoading(true)
    const response = await chatEngine.processMessage(content)
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-[400px] h-[600px] bg-ivory rounded-2xl shadow-2xl flex flex-col z-50 border border-stone/20 md:block hidden">
          {/* Header */}
          <div className="bg-charcoal text-ivory p-4 rounded-t-2xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gold rounded-full flex items-center justify-center">
                <MessageCircle className="w-5 h-5 text-navy" />
              </div>
              <div>
                <h3 className="font-semibold">Sports Memorabilia</h3>
                <p className="text-xs text-ivory/70">Online now</p>
              </div>
            </div>
            <button
          ...
          >
            ...
            {isLoading && (
              <div className="flex justify-start mb-4">
                <div className="bg-white border border-stone/20 rounded-2xl px-4 py-3">
      ...
      {/* Mobile Full Screen */}
      {isOpen && (
        <div className="fixed inset-0 bg-ivory z-50 flex flex-col md:hidden">
          {/* Header */}
          ...
          <div 
            ...
            onScroll={handleScroll}
            className="flex-1 min-h-0 overflow-y-scroll p-4 flex flex-col scrollbar-custom relative"
          >
            {messages.map((message) => (
              <ChatMessage
                key={message.id}
