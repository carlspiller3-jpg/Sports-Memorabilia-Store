...
export function AssetGenerator() {
    ...
    const processImage = async () => {
        ...
        img.onload = () => {
            ...
            if (isMount) {
                // Calculate dynamic frame thickness (4.5% of the max dimension of the crop)
                // This ensures a consistent, premium width regardless of input image resolution
                const frameThickness = Math.round(Math.max(cropWidth, cropHeight) * 0.045);
                ...
                if (frameCtx) {
                    ...
                    // Base frame fill
                    frameCtx.fillStyle = '#151517';
                    frameCtx.fillRect(0, 0, fw, fh);
                    
                    // 3D Bevel Shading for frame rails using linear gradients
                    
                    // Top rail gradient (light source highlight)
                    let grad = frameCtx.createLinearGradient(0, 0, 0, frameThickness);
                    grad.addColorStop(0, '#2d2d31');
                    grad.addColorStop(0.15, '#1e1e21');
                    ...
                    frameCtx.beginPath();
                    frameCtx.moveTo(0, 0);
                    frameCtx.lineTo(fw, 0);
                    frameCtx.lineTo(fw - frameThickness, frameThickness);
                    frameCtx.lineTo(frameThickness, frameThickness);
                    frameCtx.closePath();
                    frameCtx.fill();
                    
                    // Left rail gradient (light source highlight)
                    grad = frameCtx.createLinearGradient(0, 0, frameThickness, 0);
                    grad.addColorStop(0, '#2a2a2e');
                    grad.addColorStop(0.15, '#1d1d20');
                    grad.addColorStop(0.5, '#111113');
                    grad.addColorStop(0.85, '#161619');
                    grad.addColorStop(1, '#09090a');
                    frameCtx.fillStyle = grad;
                    frameCtx.beginPath();
                    frameCtx.moveTo(0, 0);
                    frameCtx.lineTo(frameThickness, frameThickness);
                    frameCtx.lineTo(frameThickness, fh - frameThickness);
                    frameCtx.lineTo(0, fh);
                    frameCtx.closePath();
                    frameCtx.fill();
                    
                    // Bottom rail gradient (shadow side)
                    grad = frameCtx.createLinearGradient(0, fh, 0, fh - frameThickness);
                    grad.addColorStop(0, '#0a0a0c');
                    grad.addColorStop(0.5, '#111113');
                    grad.addColorStop(0.85, '#1a1a1d');
                    grad.addColorStop(1, '#252529');
                    frameCtx.fillStyle = grad;
                    frameCtx.beginPath();
                    frameCtx.moveTo(0, fh);
                    frameCtx.lineTo(fw, fh);
                    frameCtx.lineTo(fw - frameThickness, fh - frameThickness);
                    frameCtx.lineTo(frameThickness, fh - frameThickness);
                    frameCtx.closePath();
                    frameCtx.fill();
                    
                    // Right rail gradient (shadow side)
                    grad = frameCtx.createLinearGradient(fw, 0, fw - frameThickness, 0);
                    ...
                    // Draw subtle wood grain texture (light micro-streaks)
                    frameCtx.strokeStyle = 'rgba(255, 255, 255, 0.02)';
                    frameCtx.lineWidth = 1;
                    
                    // Horizontal grain (Top & Bottom rails)
                    for (let i = 2; i < frameThickness - 2; i += 3) {
                        if (Math.random() > 0.4) {
                            frameCtx.beginPath();
                            frameCtx.moveTo(i, i);
                            frameCtx.lineTo(fw - i, i);
                            frameCtx.stroke();
                            
                            frameCtx.beginPath();
                            frameCtx.moveTo(i, fh - i);
                            frameCtx.lineTo(fw - i, fh - i);
                            frameCtx.stroke();
                        }
                    }
                    
                    // Vertical grain (Left & Right rails)
                    for (let i = 2; i < frameThickness - 2; i += 3) {
                        if (Math.random() > 0.4) {
                            frameCtx.beginPath();
                            frameCtx.moveTo(i, i);
                            frameCtx.lineTo(i, fh - i);
                            frameCtx.stroke();
                            
                            frameCtx.beginPath();
                            frameCtx.moveTo(fw - i, i);
                            frameCtx.lineTo(fw - i, fh - i);
                            frameCtx.stroke();
                        }
                    }
                    
                    // Deep dark grain lines
                    frameCtx.strokeStyle = 'rgba(0, 0, 0, 0.15)';
                    for (let i = 4; i < frameThickness - 4; i += 5) {
                        if (Math.random() > 0.3) {
                            frameCtx.beginPath();
                            frameCtx.moveTo(i, i);
                            frameCtx.lineTo(fw - i, i);
                    ...
                    // Miter joints (diagonal corner seams)
                    frameCtx.strokeStyle = 'rgba(0, 0, 0, 0.45)';
                    ...
                    // Bottom-Right miter
                    frameCtx.beginPath();
                    frameCtx.moveTo(fw, fh);
                    frameCtx.lineTo(fw - frameThickness, fh - frameThickness);
                    frameCtx.stroke();
                    
                    // Subtle highlight on top-left miter lip
                    frameCtx.strokeStyle = 'rgba(255, 255, 255, 0.05)';
                    frameCtx.lineWidth = 1;
                    frameCtx.beginPath();
                    frameCtx.moveTo(1, 0);
                    frameCtx.lineTo(frameThickness + 1, frameThickness);
                    frameCtx.stroke();
                    
                    // Inner lip depth shadow (separates frame from mount board)
                    frameCtx.strokeStyle = '#050506';
                    frameCtx.lineWidth = 3;
                    frameCtx.beginPath();
                    frameCtx.moveTo(frameThickness - 1.5, frameThickness - 1.5);
                    frameCtx.lineTo(fw - frameThickness + 1.5, frameThickness - 1.5);
                    frameCtx.moveTo(frameThickness - 1.5, frameThickness - 1.5);
                    frameCtx.lineTo(frameThickness - 1.5, fh - frameThickness + 1.5);
                    frameCtx.stroke();
                    
                    // Inner lip subtle highlight (on bottom and right rails)
                    frameCtx.strokeStyle = 'rgba(255, 255, 255, 0.1)';
                    frameCtx.lineWidth = 1.5;
                    frameCtx.beginPath();
                    frameCtx.moveTo(frameThickness, fh - frameThickness + 0.5);
                    frameCtx.lineTo(fw - frameThickness, fh - frameThickness + 0.5);
                    frameCtx.lineTo(fw - frameThickness + 0.5, frameThickness);
                    frameCtx.stroke();
